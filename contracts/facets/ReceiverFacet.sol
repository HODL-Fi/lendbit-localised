...
contract ReceiverFacet {
    ...
    function _handleLendingDebtCreation(bytes calldata reportData) internal {
        // Decode the report data into a BorrowRequest struct
        (
            ...
            reportData, (string, uint256, address, uint256, uint256, uint256, uint256, uint256, address, address, bytes)
        );

        BorrowRequest memory _request = BorrowRequest(
            action,
            positionId,
            token,
            amount,
        ...
        // Call the internal function to process the borrow request and create the loan
        LibAppStorage.StorageLayout storage s = LibAppStorage.appStorage();
        s._requestBorrow(_request, _signature);
    }
